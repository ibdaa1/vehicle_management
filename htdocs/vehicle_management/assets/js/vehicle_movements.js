...
function renderVehicleActions(vehicle) {
  ...
  // ───────────────────────────────────────────────────────────
  // الحل: role_id موجود داخل currentSession.user
  // ───────────────────────────────────────────────────────────
  const isAdminOrSuperAdmin = 
    currentSession?.user?.role_id == 1 || 
    currentSession?.user?.role_id == 2;

  console.log('DEBUG: isAdminOrSuperAdmin =', isAdminOrSuperAdmin); // للتحقق
  ...
  // 2. المركبة مستلمة من المستخدم الحالي
  else if (isCheckedOutByMe) {
    if (isAdminOrSuperAdmin) {
      actions += `
        <button class="action-button btn-return" onclick="window.returnVehicle('${vehicle.vehicle_code}')">
  ...
  // 3. صلاحيات عامة
    ...
    if (isAvailable && userHasVehicleCheckedOut) {
      actions += `
        <button class="action-button btn-disabled" disabled>
          <span class="action-icon">⚠️</span>
          <span>${t('messages.you_have_vehicle')}</span>
  ...
  // زر فتح النموذج
  if (userPermissions.allow_registration) {
    actions += `
  ...
  window.saveCoordinates = async function() {
    ...
    try {
      ...
      const result = await fetchData('/vehicle_management/api/v1/movements', {
        method: 'POST',
        body: formData
      });
