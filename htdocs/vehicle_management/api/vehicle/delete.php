...
// Handles cascading delete for child tables like vehicle_maintenance.
...
// fetch vehicle to know owner/created_by and vehicle_code
...
// Cascading delete: First delete related records in child tables
// Delete from vehicle_maintenance (based on FK constraint)
$vehicleCode = $vehicle['vehicle_code'];
$childDel = $conn->prepare("DELETE FROM vehicle_maintenance WHERE vehicle_code = ?");
if ($childDel) {
    $childDel->bind_param('s', $vehicleCode);
    if (!$childDel->execute()) {
        echo json_encode(['success'=>false,'message'=>'Failed to delete related maintenance records: ' . $childDel->error]);
        $childDel->close();
        exit;
    }
    $childDel->close();
} else {
    echo json_encode(['success'=>false,'message'=>'Server error preparing child delete']);
    exit;
}
// Add more child tables if needed, e.g.:
// $otherDel = $conn->prepare("DELETE FROM other_table WHERE vehicle_id = ?");
// ... bind and execute ...

// Now perform deletion from vehicles
