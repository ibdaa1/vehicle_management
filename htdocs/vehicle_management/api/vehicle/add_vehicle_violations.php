...
// حقل paid_by_emp_id سنملؤه تلقائياً من آخر حركة استلام
$paid_by_emp_id = null;
...
try {
    ...
    // --- 8.1 البحث عن المسؤول تلقائياً من آخر حركة استلام للمركبة ---
    $stmt = $conn->prepare("
        SELECT vm.performed_by, u.username 
        ...
        LIMIT 1
    ");
    $stmt->bind_param("ss", $vehicle_code, $violation_datetime);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if($result->num_rows > 0) {
        $responsible = $result->fetch_assoc();
        $paid_by_emp_id = $responsible['performed_by']; // تعيين المسؤول في حقل paid_by_emp_id
    }
    $stmt->close();
    
    // --- 9. إدخال المخالفة مع المسؤول في paid_by_emp_id ---
    $stmt = $conn->prepare("
        INSERT INTO vehicle_violations 
        (vehicle_id, vehicle_code, violation_datetime, violation_amount, violation_status, 
         payment_datetime, issued_by_emp_id, paid_by_emp_id, notes, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    ...
    if($paid_by_emp_id) {
        $activity_description .= " - المسؤول: {$paid_by_emp_id}";
    }
    ...
    echo json_encode([
        ...
        'responsible_info'=> [
            'emp_id' => $paid_by_emp_id,
            'field_used' => 'paid_by_emp_id'
        ]
    ], JSON_UNESCAPED_UNICODE);
