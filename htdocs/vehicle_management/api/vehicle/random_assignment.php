...
// ------------------ التحقق من أن المستخدم لديه division_id ------------------
if (empty($currentUser['division_id'])) {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'لا يمكنك استخدام القرعة لأنك غير مرتبط بأي شعبة'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$userDivisionId = intval($currentUser['division_id']);

// Only log debug info when debug=1 parameter is present
if (isset($_GET['debug']) && $_GET['debug'] === '1') {
    error_log("Debug: User ID={$currentUser['id']} | Role={$roleName} | division_id={$userDivisionId} | section_id=" . ($currentUser['section_id'] ?? 'NULL') . " | department_id=" . ($currentUser['department_id'] ?? 'NULL') . " | overrideSections=" . implode(',', $overrideSections) . " | isAdmin=" . ($isAdminRole ? 'true' : 'false'));
}
...
// - ANY user sees vehicles that belong to the user's same division (v.division_id = user.division_id).
...
// - If can_view_all_vehicles = true, see ALL operational shift vehicles (no division restriction).
...
// ------------------ قسمة المركبات حسب الشعبة (division_id) ------------------
// الشرط الأساسي: المركبة يجب أن تكون من نفس الشعبة الخاصة بالمستخدم
$where[] = "v.division_id = ?";
$params[] = $userDivisionId;
$types .= 'i';

// Visibility clause group (NEW role-based logic)
// ملاحظة: تم نقل شرط الشعبة إلى الأعلى ليكون أساسياً للجميع
$visibilityClauses = [];
if ($isAdminRole) {
    // Admin: يرى جميع مركبات الشعبة
    $visibilityClauses[] = "1=1";
} else {
    // Non-admin: regular_user or custom_user
    if ($isCustomUser && !empty($overrideSections)) {
        // custom_user: يرى مركبات الشعبة في الأقسام المحددة
        ...
        // regular_user: يرى مركبات الشعبة في قسمه فقط
...
if (empty($visibilityClauses)) {
    ...
    echo json_encode(['success' => false, 'message' => 'لا صلاحية رؤية أي سيارات في شعبتك.'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
// Optional GET filter for specific department_id (restrict results)
...
if (!$availableVehicle) {
    echo json_encode(['success' => false, 'message' => 'لا سيارات متاحة في شعبتك (' . $userDivisionId . ') للقرعة.'], JSON_UNESCAPED_UNICODE);
    exit;
}

// ------------------ Assignment authorization checks (NEW role-based) ------------------
// Rule: 
// - Admin roles: Can assign any available vehicle within division
// - regular_user/custom_user: Can assign (they use random assignment within their division)
$canAssignThis = false;
if ($isAdminRole) {
    $canAssignThis = true; // Admin can assign any vehicle within division
} elseif ($isRegularUser || $isCustomUser) {
    // Regular and custom users can assign via random (already filtered by division and visibility)
    $canAssignThis = true;
} else {
    $canAssignThis = false;
}
...
$notes = 'قرعة عشوائية - المستخدم: ' . $createdBy . ' - الشعبة: ' . $userDivisionId;
...
if ($insertStmt->execute()) {
    echo json_encode([
        'success' => true,
        'message' => 'تم تعيين ' . $vehicleCode . ' عبر القرعة من شعبتك.',
        'vehicle' => [
            'code' => $vehicleCode,
            'type' => $availableVehicle['type'] ?? '',
            'driver_name' => $availableVehicle['driver_name'] ?? '',
            'driver_phone' => $availableVehicle['driver_phone'] ?? '',
            'division_id' => $availableVehicle['division_id'] ?? '',
            'division_name' => $availableVehicle['division_name'] ?? ''
        ],
        'user_division' => $userDivisionId,
