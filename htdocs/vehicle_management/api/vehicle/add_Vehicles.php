...
$emp_id_field = trim((string)(input_get('emp_id', $json) ?? '')); // emp_id للسائق (مطلوب، لا fallback)
...
if ($vehicle_code === '') {
    echo json_encode(['success' => false, 'message' => 'رقم المركبة مطلوب (vehicle_code required)']);
    exit;
}
if ($emp_id_field === '') {
    echo json_encode(['success' => false, 'message' => 'الرمز الوظيفي لسائق المركبة مطلوب (emp_id required)']);
    exit;
}
// تحقق من سنة الصنع
if ($manufacture_year !== null && (!is_numeric($manufacture_year) || intval($manufacture_year) < 1900 || intval($manufacture_year) > 2099)) {
    echo json_encode(['success' => false, 'message' => 'سنة الصنع يجب أن تكون بين 1900 و 2099']);
    exit;
}
$isUpdate = !empty($vehicle_id);
if ($isUpdate) {
    if (!has_perm($user, 'vehicles_edit')) {
        echo json_encode(['success' => false, 'message' => 'غير مسموح بالتعديل (Forbidden)']);
        exit;
    }
} else {
    if (!has_perm($user, 'vehicles_add')) {
        echo json_encode(['success' => false, 'message' => 'غير مسموح بالإضافة (Forbidden)']);
        exit;
    }
}
...
// emp_id للسائق (من الإدخال اليدوي)
$emp_to_store = $emp_id_field;
// emp_id للمنشئ/المحدث (من الجلسة)
$session_emp_id = $user['emp_id'] ?? null;
if (empty($session_emp_id)) {
    echo json_encode(['success' => false, 'message' => 'خطأ في الجلسة: emp_id غير متوفر']);
    exit;
}
try {
    if ($isUpdate) {
        $fields = [
            ...
            'notes' => $notes,
            'updated_by' => $session_emp_id // من الجلسة (emp_id)
        ];
        ...
        $stmt->close();
        echo json_encode(['success' => true, 'message' => 'تم التحديث بنجاح', 'vehicle_id' => intval($vehicle_id), 'affected_rows' => $affected]);
        exit;
    } else {
        $values = [
            'vehicle_code' => $vehicle_code,
            ...
            'vehicle_mode' => $vehicle_mode,
            'notes' => $notes,
            'created_by' => $session_emp_id // من الجلسة (emp_id)
        ];
        ...
        if (!$stmt->execute()) {
            ...
            if ($errno === 1062) {
                // duplicate entry
                throw new Exception('رقم المركبة موجود مسبقًا (Duplicate entry)');
            }
            throw new Exception('Execute failed: ' . $err);
        }
        $newId = $stmt->insert_id;
        $stmt->close();
        echo json_encode(['success' => true, 'message' => 'تم إضافة المركبة بنجاح', 'vehicle_id' => intval($newId)]);
        exit;
    }
} catch (Throwable $ex) {
    error_log('add_Vehicles error: ' . $ex->getMessage());
    // user-facing message (no SQL internals)
    echo json_encode(['success' => false, 'message' => 'خطأ في الخادم: ' . $ex->getMessage()]);
    exit;
}
?>