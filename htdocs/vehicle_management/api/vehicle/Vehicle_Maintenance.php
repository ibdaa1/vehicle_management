...
try {
    ...
    if ($action === 'list') {
        ...
        $from = $_GET['from_date'] ?? '';
        $to = $_GET['to_date'] ?? '';
        $visit_date_single = $_GET['visit_date'] ?? ''; // New: Single date filter for Visit Date
        ...
        if ($q !== '') {
            $qLike = '%' . $q . '%';
            $where[] = "(vm.vehicle_code LIKE ? OR vm.location LIKE ? OR v.driver_name LIKE ? OR v.driver_phone LIKE ? OR v.status LIKE ? OR CAST(v.manufacture_year AS CHAR) LIKE ? OR v.emp_id LIKE ? OR v.type LIKE ?)";
            $params[] = $qLike; $params[] = $qLike; $params[] = $qLike; $params[] = $qLike; $params[] = $qLike; $params[] = $qLike; $params[] = $qLike; $params[] = $qLike;
            $types .= 'ssssssss';
        }
        ...
        // Date filters - Support both range (from/to) and single date
        if ($from !== '') {
            $where[] = "DATE(vm.visit_date) >= ?";
            $params[] = $from;
            $types .= 's';
        }
        ...
        if ($visit_date_single !== '') {
            $where[] = "DATE(vm.visit_date) = ?";
            $params[] = $visit_date_single;
            $types .= 's';
        }
        ...
        $countSql = "SELECT COUNT(*) AS cnt FROM vehicle_maintenance vm LEFT JOIN vehicles v ON vm.vehicle_code = v.vehicle_code $whereSql";
        ...
        $sql = "SELECT vm.*, v.driver_name, v.driver_phone, v.status, v.manufacture_year, v.emp_id, v.type, v.department_id, v.section_id, v.division_id 
                FROM vehicle_maintenance vm 
                LEFT JOIN vehicles v ON vm.vehicle_code = v.vehicle_code 
                $whereSql 
                ORDER BY vm.visit_date DESC, vm.id DESC 
                LIMIT ? OFFSET ?";
    ...
    if ($action === 'get') {
        ...
        $stmt = $conn->prepare("
            SELECT vm.*, v.driver_name, v.driver_phone, v.status, v.manufacture_year, v.emp_id, v.type, v.department_id, v.section_id, v.division_id 
            FROM vehicle_maintenance vm 
            LEFT JOIN vehicles v ON vm.vehicle_code = v.vehicle_code 
            WHERE vm.id = ? LIMIT 1
        ");
