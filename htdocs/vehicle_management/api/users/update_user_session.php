...
if ($new_password !== '') {
    ...
    if ($st) {
        ...
        $errors[] = 'Server error (password check).';
    }
}
...
if (!empty($fields)) {
    ...
    if (!$stmt->execute()) {
        ...
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Server error (execute).'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    $stmt->close();
}
...
// refresh session data from DB and update $_SESSION['user']
...
if ($s) {
    ...
    if ($user) {
        // update session structure consistently
        $_SESSION['user'] = $user;
    }
} else {
    // could not refresh, but still return success
    $user = null;
}

echo json_encode(['success' => true, 'message' => 'تم التحديث', 'user' => $user], JSON_UNESCAPED_UNICODE);
exit;
?>