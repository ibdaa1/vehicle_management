...
// Debug-enabled admin update endpoint with password support.
// Usage: POST to this URL with form fields (id, display_name, email, phone, department_id, section_id, division_id, role_id, is_active, password, ...)
...
try {
    ...
    if (!empty($newPassword)) {
        ...
        // Add password_hash to fields to update
    ...
    foreach ($updatable as $field => $type) {
        if (isset($_POST[$field])) {
            ...
            // Special handling for password_hash - if empty, skip (don't update)
            if ($field === 'password_hash' && !$hasPasswordUpdate) {
                continue;
            }
    ...
    $debug_info['password_updated'] = $hasPasswordUpdate;
